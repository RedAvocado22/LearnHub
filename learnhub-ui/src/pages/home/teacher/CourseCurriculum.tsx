...
export default function CourseCurriculum({ course }: CourseCurriculumProps) {
    ...
    useEffect(() => {
        const fetchChapters = async () => {
            try {
                const resp = await API.get(`/chapters/${course.id}`);
                if (resp.status === 200) {
    ...
    const handleAddLesson = async (chapterId: number) => {
        ...
        if (value) {
            try {
                const resp = await API.post(`/chapters/lessons/${chapterId}`, { title: value });
                if (resp.status === 200) {
    ...
    const handleRemoveLesson = async (id: number) => {
        ...
        if (isConfirmed) {
            try {
                const resp = await API.delete(`/chapters/lessons/${id}`);
                if (resp.status === 200) {
